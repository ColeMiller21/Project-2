        ...
        if (email && password) {
            ...
            db.Users.findAll({
                ...
                if (data.length > 0) {
                    ...
                    return res.status(200).json({ success: true, response: "The user has been logged in." });
                } else {
                    console.log("no");
                    // If the data returned has nothing than a status code of 400 is sent to signify a failure
                    return res.status(401).json({ success: false, response: "This user does not exist.  Please make sure the credentials are correct." });
                }
            });
    ...
    app.post("/user/signup", function (req, res) {
        ...
        if (email && password && username) {
            ...
            db.Users.findAll({
                ...
                if (data.length > 0) {
                    return res.status(401).json({ success: false, response: "This email is already in use." });
                } else {
                    ...
                    db.Users.create({
                        ...
                        if (err) {
                            return res.status(500).json({ success: false, response: "An error has occurred." });
                        }
                    }).then(function () {
                        // A status code of 200 is sent to signify a success
                        return res.status(200).json({ success: true, response: "This user has been created." });
                    })
                }
            });
        }
